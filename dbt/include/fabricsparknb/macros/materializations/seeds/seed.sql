{% macro fabricsparknb__get_binding_char() %}
  {{ return('?' if target.method == 'odbc' else '%s') }}
{% endmacro %}


{% macro fabricsparknb__reset_csv_table(model, full_refresh, old_relation, agate_table) %}
    {% if old_relation %}
...
{% macro fabricsparknb__get_batch_size() %}
  {{ return(500) }}
{% endmacro %}

{% macro fabricsparknb__load_csv_rows(model, agate_table) %}
...
{% macro fabricsparknb__create_csv_table(model, agate_table) %}
  {%- set column_override = model['config'].get('column_types', {}) -%}
